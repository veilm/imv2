...
static void consume_internal_event(struct imv *imv, struct internal_event *event)
{
  if (event->type == NEW_IMAGE) {
    ...
    const bool valid = imv_thumbs_handle_loaded(imv->thumbs,
        event->data.thumb_loaded.index,
        event->data.thumb_loaded.generation, event->data.thumb_loaded.image);
    if (valid && !event->data.thumb_loaded.image) {
      const char *path = imv_navigator_at(imv->navigator,
          event->data.thumb_loaded.index);
      if (path) {
